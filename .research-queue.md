...
**Open: 0 critical, 0 high, 4 medium** (Cycle 133)

Last scan: Cycle 133 — fix 4 medium [PD-023, PD-025, PD-026, PD-031] + system-level research (normalize)
...
- **NR-001** — `src/pre-summarize/index.ts:138` — Stale batch_id on skipped items; release sets status='ready' but doesn't clear batch_id (dirty data, not functional break)
- **NR-002** — `src/pre-summarize/index.ts:122,133` — Wasteful claim-and-release cycle; urgency/density heuristics could move into SQL WHERE clause
- **NR-003** — `src/index.ts:135-148` — `news` source type silently dropped in poll loop; adapter exists but is never imported or wired (dead code)
- **NR-004** — `src/pre-summarize/index.ts:88` — parseLabeledOutput regex merges content when LLM skips a label in batch output
