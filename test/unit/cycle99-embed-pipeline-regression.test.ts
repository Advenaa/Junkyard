...
const src = readFileSync(
  new URL('../../src/embed-pipeline.ts', import.meta.url), 'utf-8',
);
...
describe('EM-024 — ALLOWED_TABLES constant exists with summaries and reports', () => {
  it('defines ALLOWED_TABLES with summaries and reports keys', () => {
    assert.ok(
      src.includes('const ALLOWED_TABLES'),
      'embed-pipeline.ts must define an ALLOWED_TABLES constant',
    );
    assert.match(
      src,
    ...
    assert.ok(
      src.includes("summaries:") || src.includes("summaries :"),
      'ALLOWED_TABLES must include a summaries key',
    );
    assert.ok(
      src.includes("reports:") || src.includes("reports :"),
      'ALLOWED_TABLES must include a reports key',
    );
  });
});
...
describe('EM-024 — each allowed table has body in its column list', () => {
  it('summaries and reports both list body as an allowed column', () => {
    ...
    assert.ok(
      summariesMatch![1].includes("'body'"),
      "summaries column list must include 'body'",
    );

    const reportsMatch = block.match(/reports:\s*\[([^\]]*)\]/);
    assert.ok(reportsMatch, 'reports key must have an array value');
    assert.ok(
      reportsMatch![1].includes("'body'"),
      "reports column list must include 'body'",
    );
  });
});
...
describe('EM-024 — fetchUnembedded validates table/column before query', () => {
  it('throws on invalid table or column combination', () => {
    ...
    assert.ok(
      fnBody.includes('ALLOWED_TABLES[tableName]'),
      'fetchUnembedded must look up tableName in ALLOWED_TABLES',
    );
    assert.ok(
    ...
    assert.ok(
      ...
      'fetchUnembedded must throw on invalid table/column',
    );
  });
});
