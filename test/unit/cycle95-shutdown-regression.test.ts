...
describe('SL-012 — force-exit timeout upgraded to 60s with .unref()', () => {
  it('shutdown section contains 60_000 (not 30_000)', () => {
    ...
    assert.ok(
      shutdownSection.includes('60_000'),
      'force-exit timeout must be 60_000',
    );
    assert.ok(
      !shutdownSection.includes('30_000'),
      'old 30_000 timeout must not be present in shutdown section',
    );
  });
  ...
  it('setTimeout call is paired with .unref()', () => {
    ...
    assert.ok(
      match,
      'setTimeout(() => process.exit(1), 60_000).unref() must appear as a single chained expression',
    );
  });
  ...
  it('process.exit(1) is inside the setTimeout callback (hard kill)', () => {
    ...
    assert.ok(
      snippet.includes('process.exit(1)'),
      'process.exit(1) must be inside the setTimeout callback expression',
    );
  });
});
