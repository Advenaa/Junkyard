...
export function createRetention(pool: Pool, log: Logger) {
  async function run(): Promise<RetentionResult> {
    ...
    const itemsResult = await pool.query(`DELETE FROM items WHERE status = 'processed' AND created_at < $1`, [
      thirtyDaysAgo,
    ]);
    ...
    const summariesResult = await pool.query(`DELETE FROM summaries WHERE created_at < $1`, [ninetyDaysAgo]);
    ...
    const mentionsResult = await pool.query(`DELETE FROM entity_mentions WHERE created_at < $1`, [ninetyDaysAgo]);
    ...
    const sentimentResult = await pool.query(`DELETE FROM entity_sentiment_daily WHERE date < $1`, [cutoffDate]);
    ...
    for (const [targetType, sourceTable] of [
      ['item', 'items'],
      ['summary', 'summaries'],
    ] as const) {
      let deleted: number;
    ...
    const sessionsResult = await pool.query(`DELETE FROM sessions WHERE expires_at < $1`, [now]);
